 ...
 */
class GetAllIslandsStatisticsUseCase @Inject constructor(
    ...
    suspend operator fun invoke(): Result<IslandStatistics> {
        return try {
            val activeCount = islandRepository.getActiveIslandsCount()
                .getOrElse { return Result.failure(it) }

            val typeStats = islandRepository.getIslandTypeStats()
                .getOrElse { return Result.failure(it) }

            val maintenanceStats = islandRepository.getMaintenanceStats()
                .getOrElse { return Result.failure(it) }
    ...
    suspend fun getTypeSpecificStatistics(islandType: IslandType): Result<TypeSpecificStatistics> {
        return try {
            val typeIslands = islandRepository.getIslandsByType(islandType)
                .getOrElse { return Result.failure(it) }
     ...
     */
    private fun calculateEfficiency(island: Island): Int {
