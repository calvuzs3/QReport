    ...
    suspend fun setPrimaryContactForClient(clientId: String, contactId: String): QrResult<Unit, QrError> {
        return try {
            ...
            if (contact.clientId != clientId) {
                ...
                return QrResult.Error(QrError.ValidationError.InvalidOperation(QrError.ContactsError.ValidationError.ContactDoesntBelongToClient))
            }
    ...
    suspend fun removePrimaryStatus(contactId: String): QrResult<Unit, QrError> {
        return try {
            ...
            if (otherActiveContacts.isEmpty()) {
                ...
                return QrResult.Error(QrError.ValidationError.InvalidOperation(QrError.ContactsError.ValidationError.CannotRemovePrimaryFlag))
            }
    ...
    suspend fun autoAssignPrimaryIfMissing(clientId: String): QrResult<Contact?, QrError> {
        return try {
            if (clientId.isBlank()) {
                ...
                return QrResult.Error(QrError.ValidationError.InvalidOperation(QrError.ContactsError.ValidationError.IdMandatory()))
            }
