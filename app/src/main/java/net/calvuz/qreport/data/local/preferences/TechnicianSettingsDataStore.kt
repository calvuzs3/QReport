package net.calvuz.qreport.data.local.preferences
...
class TechnicianSettingsDataStore @Inject constructor(
    ...
    fun getTechnicianInfo(): Flow<TechnicianInfo> =
        context.technicianSettingsDataStore.data
            .catch { exception ->
                Timber.e(exception, "Errore lettura technician settings, usando valori default")
    ...
    suspend fun updateTechnicianInfo(technicianInfo: TechnicianInfo) {
        try {
            ...
            Timber.d("Technician info updated in DataStore")
        } catch (e: Exception) {
            Timber.e(e, "Errore aggiornamento technician info in DataStore")
            throw e
        }
    }
    ...
    suspend fun clearTechnicianInfo() {
        try {
            ...
            Timber.d("Technician info cleared from DataStore")
        } catch (e: Exception) {
            Timber.e(e, "Errore clearing technician info from DataStore")
            throw e
        }
    }
    ...
    fun getLastUpdatedTimestamp(): Flow<Long?> =
        context.technicianSettingsDataStore.data
            .catch { exception ->
                Timber.e(exception, "Errore lettura last updated timestamp")
                emit(emptyPreferences())
            }
            .map { preferences ->
    ...
    suspend fun exportForBackup(): Map<String, String> {
        return try {
            ...
            Timber.e(e, "Errore export technician settings for backup")
            emptyMap()
        }
    }
    ...
    suspend fun importFromBackup(backupData: Map<String, String>) {
        try {
            if (backupData.isEmpty()) {
                Timber.w("Backup data is empty for technician settings")
                return
            }
            ...
            Timber.d("Technician settings imported successfully from backup")

        } catch (e: Exception) {
            Timber.e(e, "Errore import technician settings from backup")
            throw e
        }
    }
    ...
    suspend fun getTechnicianSettingsSummary(): Map<String, Any> {
        return try {
            ...
            Timber.e(e, "Errore ottenendo technician settings summary")
