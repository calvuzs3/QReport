...
import net.calvuz.qreport.data.local.preferences.TechnicianSettingsDataStore
...
class TechnicianSettingsRepositoryImpl @Inject constructor(
    ...
    override fun getTechnicianInfo(): Flow<TechnicianInfo> {
        return technicianSettingsDataStore.getTechnicianInfo()
            .catch { exception ->
                Timber.e(exception, "Errore lettura technician info")
    ...
    override suspend fun updateTechnicianInfo(technicianInfo: TechnicianInfo): Result<Unit> {
        return try {
            ...
            Timber.i("Technician info updated successfully")
            Result.success(Unit)

        } catch (e: Exception) {
            Timber.e(e, "Errore aggiornamento technician info")
            Result.failure(e)
        }
    }
    ...
    override fun hasTechnicianData(): Flow<Boolean> {
        return technicianSettingsDataStore.getTechnicianInfo()
            ...
            .catch { exception ->
                Timber.e(exception, "Errore verifica technician data")
    ...
    override suspend fun resetToDefault(): Result<Unit> {
        return try {
            ...
            Timber.i("Technician settings reset to default")
            Result.success(Unit)

        } catch (e: Exception) {
            Timber.e(e, "Errore reset technician settings")
            Result.failure(e)
        }
    }
    ...
    override suspend fun exportForBackup(): Map<String, String> {
        return try {
            ...
            Timber.i("Exported ${backupMap.size} technician settings for backup")
            backupMap

        } catch (e: Exception) {
            Timber.e(e, "Errore export technician settings")
            emptyMap()
        }
    }
    ...
    override suspend fun importFromBackup(backupData: Map<String, String>): Result<Unit> {
        return try {
            if (backupData.isEmpty()) {
                Timber.w("Backup data is empty, skipping import")
            ...
            Timber.i("Technician settings imported successfully from backup")
            Result.success(Unit)

        } catch (e: Exception) {
            Timber.e(e, "Errore import technician settings")
            Result.failure(e)
        }
    }
    ...
    private fun validateTechnicianInfo(technicianInfo: TechnicianInfo): List<String> {
        Timber.i("Validating technician info")
    ...
    private fun validateBackupData(backupData: Map<String, String>): List<String> {
        Timber.i("Validating backup data structure")
    ...
    suspend fun getTechnicianSettingsSummary(): Map<String, String> {
        return try {
            ...
            Timber.e(e, "Errore ottenendo summary technician settings")
