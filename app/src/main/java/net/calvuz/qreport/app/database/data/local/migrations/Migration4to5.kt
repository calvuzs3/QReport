 ...
 *
 * Tables are created empty — no default rows. The original seed (one
 * company's specific module/criticality/checklist content) was moved out of
 * the repo to `company-checkup-seed-data.kt` (gitignored, project root); use
 * the master-data management screens (Settings > Impostazioni Checkup) to
 * populate these tables for a given install, or paste that file's
 * `seedCompanyMasterData(db, now)` into a throwaway local migration to
