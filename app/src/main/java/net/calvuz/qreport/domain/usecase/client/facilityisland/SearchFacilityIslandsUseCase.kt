 ...
 */
class SearchFacilityIslandsUseCase @Inject constructor(
     ...
     */
    suspend operator fun invoke(query: String): Result<List<FacilityIsland>> {
        return try {
            // Validazione input
            if (query.isBlank()) {
            ...
            facilityIslandRepository.searchIslands(query.trim())
                .map { islands ->
                    // Ordina per relevanza
                    islands.sortedWith(
                        compareBy<FacilityIsland> { island ->
     ...
     */
    suspend fun findBySerialNumber(serialNumber: String): Result<FacilityIsland?> {
        return try {
            if (serialNumber.isBlank()) {
                return Result.failure(IllegalArgumentException("Serial number non può essere vuoto"))
            }

            facilityIslandRepository.getIslandBySerialNumber(serialNumber.trim())
        } catch (e: Exception) {
            Result.failure(e)
        }
    }
     ...
     */
    suspend fun filterByType(islandType: IslandType): Result<List<FacilityIsland>> {
        return try {
            facilityIslandRepository.getIslandsByType(islandType)
                .map { islands ->
                    islands.sortedWith(
                        compareBy<FacilityIsland> { it.serialNumber.lowercase() }
                    )
                }
        } catch (e: Exception) {
            Result.failure(e)
     ...
     */
    suspend fun filterByClient(clientId: String): Result<List<FacilityIsland>> {
        return try {
            if (clientId.isBlank()) {
                return Result.failure(IllegalArgumentException("ID cliente non può essere vuoto"))
            }

            facilityIslandRepository.getIslandsByClient(clientId)
                .map { islands ->
                    islands.sortedWith(
                        compareBy<FacilityIsland> { it.islandType.name }
     ...
     */
    suspend fun advancedSearch(searchCriteria: IslandSearchCriteria): Result<List<FacilityIsland>> {
        return try {
            var result = facilityIslandRepository.getActiveIslands()
     ...
     */
    suspend fun getIslandsRequiringAttention(currentTime: Instant? = null): Result<Map<String, List<FacilityIsland>>> {
        return try {
            val timestamp = currentTime ?: Clock.System.now()
            val categories = mutableMapOf<String, List<FacilityIsland>>()

            // Isole con manutenzione scaduta
            val maintenanceOverdue = facilityIslandRepository.getIslandsRequiringMaintenance(timestamp)
                .getOrElse { return Result.failure(it) }
            categories["maintenance_overdue"] = maintenanceOverdue

            // Isole con garanzia in scadenza (prossimi 30 giorni)
            val warrantyExpiringSoon = facilityIslandRepository.getIslandsUnderWarranty(timestamp)
                .getOrElse { return Result.failure(it) }
                .filter { island ->
     ...
     */
    suspend fun searchBySerialPattern(pattern: String): Result<List<FacilityIsland>> {
        return try {
            if (pattern.isBlank()) {
