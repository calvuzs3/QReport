 ...
 */
class GetFacilityIslandsByFacilityUseCase @Inject constructor(
     ...
     */
    suspend operator fun invoke(facilityId: String): Result<List<FacilityIsland>> {
        return try {
            // 1. Validazione input
            ...
            facilityIslandRepository.getIslandsByFacility(facilityId)
                .map { islands ->
                    islands.sortedWith(
                        compareBy<FacilityIsland> { it.islandType.name } // Prima per tipo
     ...
     */
    fun observeIslandsByFacility(facilityId: String): Flow<List<FacilityIsland>> {
        return facilityIslandRepository.getIslandsByFacilityFlow(facilityId)
            .map { islands ->
                islands.sortedWith(
    ...
    suspend fun getActiveIslandsByFacility(facilityId: String): Result<List<FacilityIsland>> {
        return try {
            ...
            facilityIslandRepository.getActiveIslandsByFacility(facilityId)
                .map { islands ->
                    islands.sortedWith(
                        compareBy<FacilityIsland> { it.islandType.name }
                            .thenBy { it.customName?.lowercase() ?: it.serialNumber.lowercase() }
     ...
     */
    suspend fun getIslandsByType(facilityId: String, islandType: IslandType): Result<List<FacilityIsland>> {
        return try {
            if (facilityId.isBlank()) {
                return Result.failure(IllegalArgumentException("ID facility non può essere vuoto"))
            }
     ...
     */
    suspend fun getIslandsDueMaintenance(facilityId: String, currentTime: Instant? = null): Result<List<FacilityIsland>> {
        return try {
            if (facilityId.isBlank()) {
                return Result.failure(IllegalArgumentException("ID facility non può essere vuoto"))
            }
            ...
            val allMaintenanceRequired = facilityIslandRepository.getIslandsRequiringMaintenance(timestamp)
     ...
     */
    suspend fun getIslandsUnderWarranty(facilityId: String, currentTime: Instant? = null): Result<List<FacilityIsland>> {
        return try {
            if (facilityId.isBlank()) {
                return Result.failure(IllegalArgumentException("ID facility non può essere vuoto"))
            }
            ...
            val timestamp = currentTime ?: Clock.System.now()
            val allUnderWarranty = facilityIslandRepository.getIslandsUnderWarranty(timestamp)
                .getOrElse { return Result.failure(it) }
    ...
    suspend fun getIslandsCount(facilityId: String): Result<Int> {
        return try {
            ...
            facilityIslandRepository.getIslandsCountByFacility(facilityId)
        } catch (e: Exception) {
            Result.failure(e)
        }
    }
