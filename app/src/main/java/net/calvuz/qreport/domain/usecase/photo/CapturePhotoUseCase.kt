...
class CapturePhotoUseCase @Inject constructor(
            ...
            when (val saveResult = photoStorageManager.savePhoto(
                ...
                is PhotoSaveResult.Success -> {
                    ...
                    try {
                        ...
                        photoStorageManager.deletePhoto(photo)
    ...
    suspend fun verifyPhotoExists(photo: Photo): PhotoResult<Boolean> {
        return try {
            val exists = photoStorageManager.photoExists(photo)
     ...
     */
    suspend fun getStorageInfo(): PhotoResult<net.calvuz.qreport.data.photo.StorageInfo> {
        return try {
